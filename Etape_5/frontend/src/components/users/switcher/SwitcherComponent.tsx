...
const AddressModeSwitcher: FunctionComponent<Props> = ({ addressResponse, onDelete }) => {
    ...
    return (
        <div className="container mt-5 p-4 border rounded">
            <hr />
            {isAdmin && isEditing && <AddressForm addressRequest={null} addressResponse={addressResponse} onSubmit={handleEdit} isModifyMode={true} userToCreate={null} />}
            {!isEditing && addressResponse && <AddressDisplay addressResponse={address || addressResponse} />}
            {isAdmin && <button className="btn btn-secondary mr-2" onClick={() => setIsEditing(!isEditing)}>
                {isEditing ? "Cancel" : "Modify"}
            </button>}
            {isAdmin && <button className="btn btn-danger" onClick={() => handleDelete(addressResponse.id)}>Delete</button>}
            <hr />
        </div >
    );
};
